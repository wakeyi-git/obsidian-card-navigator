...
export const ko = {
	...
	settings: {
		...
		grouping: {
			title: '그룹화',
			enableGrouping: '그룹화 활성화',
			enableGroupingDescription: '다양한 기준으로 카드를 그룹화합니다',
			groupBy: '그룹화 기준',
			groupByDescription: '카드를 어떻게 그룹화할지 선택합니다',
			criteria: {
				folder: '폴더',
				tag: '태그',
				dateYear: '날짜 (연도)',
			...
			monthFormat: (year: number, month: string) => `${year}년 ${month}`,
			...
			dateBasis: '날짜 기준',
			dateBasisDescription: '생성 날짜 또는 수정 날짜 사용',
			dateBasisOptions: {
				created: '생성 날짜',
				modified: '수정 날짜',
			},
			tagMode: '태그 모드',
			tagModeDescription: '첫 번째 태그만 사용하거나 모든 태그 사용',
			tagModeOptions: {
				first: '첫 번째 태그만',
				all: '모든 태그 (파일이 여러 그룹에 나타날 수 있음)',
			},
			propertyName: '속성 이름',
			propertyNameDescription: '그룹화할 프론트매터 속성',
			propertyNamePlaceholder: 'status',
			showFullFolderPath: '전체 폴더 경로 표시',
			showFullFolderPathDescription: '폴더 이름만이 아닌 전체 폴더 경로를 표시합니다',
			sortGroupsBy: '그룹 정렬 기준',
			sortGroupsByDescription: '그룹 자체를 어떻게 정렬할지 선택합니다',
			sortGroupsOptions: {
				name: '이름',
				fileCount: '파일 개수',
				latestFile: '최근 파일',
		...
		// 2D 매트릭스 그룹화 (아이젠하워 매트릭스)
		matrix2D: {
			title: '2D 매트릭스 그룹화',
			description: '카드를 2D 매트릭스 레이아웃으로 그룹화합니다 (예: 아이젠하워 매트릭스)',
			enabled: '2D 매트릭스 활성화',
			enabledDescription: '두 가지 속성 기준으로 카드를 2D 매트릭스 레이아웃에 표시합니다',
			primaryAxis: '기본 축 (가로)',
			primaryAxisDescription: '가로 축에 사용할 속성',
			secondaryAxis: '보조 축 (세로)',
			secondaryAxisDescription: '세로 축에 사용할 속성',
			propertyName: '속성 이름',
			propertyNamePlaceholder: '예: urgency',
			propertyValues: '속성 값',
			propertyValuesDescription: '쉼표로 구분된 값 목록 (표시 순서대로)',
			propertyValuesPlaceholder: '예: Urgent, Not Urgent',
			axisLabel: '축 레이블',
			axisLabelPlaceholder: '예: 긴급도',
			showUnclassifiedSection: '미분류 섹션 표시',
			showUnclassifiedSectionDescription: '속성값이 없는 파일을 상단의 별도 섹션에 표시합니다',
			unclassifiedSectionTitle: '미분류 섹션 제목',
			unclassifiedSectionTitlePlaceholder: '미분류',
			enableDragDropPropertyChange: '드래그 앤 드롭 속성 변경 활성화',
			enableDragDropPropertyChangeDescription: '카드를 다른 셀로 드래그하여 파일 속성을 변경할 수 있습니다',
			cellMinWidth: '셀 최소 너비',
			cellMinWidthDescription: '각 매트릭스 셀의 최소 너비 (px)',
			cellMinHeight: '셀 최소 높이',
			cellMinHeightDescription: '각 매트릭스 셀의 최소 높이 (px)',
			cardMinWidth: '카드 최소 너비',
			cardMinWidthDescription: '매트릭스 셀 내 카드의 최소 너비 (px)',
			presets: {
				title: '매트릭스 프리셋',
				eisenhowerMatrix: '아이젠하워 매트릭스',
				eisenhowerDescription: '긴급함/긴급하지 않음 × 중요함/중요하지 않음',
				custom: '사용자 정의 매트릭스',
			},
			dragDropHint: '카드를 셀로 드래그하여 속성을 변경하세요',
			unclassifiedHint: '속성값이 없는 카드는 여기에 표시됩니다',
			propertyUpdated: (fileName: string, props: string) => `${fileName} 업데이트됨: ${props}`,
			propertyUpdateFailed: '속성 업데이트 실패',
		},
	},
	...
	toolbar: {
		...
		groupingToggle: '그룹화 토글',
		groupingEnable: '그룹화 활성화',
		groupingDisable: '그룹화 비활성화',
		pinToggle: '고정 파일 표시 토글',
		pinShow: '고정 파일 항상 표시',
		...
		configureMultiSort: '다단계 정렬 구성',
		multiSortModalDescription: '드래그 앤 드롭으로 정렬 레벨을 재정렬하세요. 파일은 먼저 레벨 1, 그 다음 레벨 2 등의 순서로 정렬됩니다.',
		sortLevel: (level: number) => `레벨 ${level}`,
		disableMultiSort: '다단계 정렬 비활성화',
		hoverActions: {
			pin: '고정',
			star: '별표',
			tag: '태그 추가',
	...
	settingsTab: {
		...
		debugMode: {
			...
			categories: {
				...
				grouping: { label: '그룹화', description: '카드 그룹화, 그룹 생성, 접기/펼치기' },
			},
		...
		presetSettings: {
			...
			editFolderMappingTitle: '폴더 매핑 편집',
			editTagMappingTitle: '태그 매핑 편집',
			editPropertyMappingTitle: '속성 매핑 편집',
			editDateMappingTitle: '날짜 매핑 편집',
			folderMappingUpdated: '폴더 매핑이 업데이트되었습니다',
			tagMappingUpdated: '태그 매핑이 업데이트되었습니다',
			propertyMappingUpdated: '속성 매핑이 업데이트되었습니다',
			dateMappingUpdated: '날짜 매핑이 업데이트되었습니다',

			// 적용 범위 (프리셋 카테고리)
			applyScopeButton: '적용 범위',
			applyScopeTitle: '프리셋 적용 범위',
			applyScopeDescription: '이 프리셋이 자동 적용될 때 어떤 설정 카테고리를 적용할지 선택하세요. 선택되지 않은 카테고리는 현재 전역 설정을 유지합니다.',
			applyScopeUpdated: '프리셋 적용 범위가 업데이트되었습니다',
			selectAll: '전체 선택',
			deselectAll: '전체 해제',
			categoryMode: '모드',
			categoryModeDesc: '폴더/태그 모드 및 모드별 설정',
			categoryGrouping: '그룹화',
			categoryGroupingDesc: '그룹화 기준, 그룹 정렬 (2D 매트릭스 제외)',
			categoryMatrix2D: '2D 매트릭스',
			categoryMatrix2DDesc: '2D 매트릭스 그룹화 설정 (아이젠하워 매트릭스 등)',
			categoryPin: '핀',
			categoryPinDesc: '핀 고정 파일 설정 및 핀 그룹 표시',
			categorySort: '정렬',
			categorySortDesc: '카드 정렬 기준 및 순서',
			categoryCardContent: '카드 내용',
			categoryCardContentDesc: '헤더/바디/푸터 콘텐츠 타입 및 렌더링 모드',
			categoryCardStyle: '카드 스타일',
			categoryCardStyleDesc: '카드 색상, 테두리 및 스타일',
			categoryLayout: '레이아웃',
			categoryLayoutDesc: '카드 크기, 간격 및 레이아웃 모드',
			categoryInteraction: '상호작용',
			categoryInteractionDesc: '호버 액션, 클릭 동작 및 드래그 앤 드롭',
			priorityTypeOptions: {
				folder: '폴더',
				tag: '태그',
				property: '속성',
		...
		cardSettings: {
			...
			contentType: {
				...
				imageThumbnail: '이미지 섬네일',
			},
			imageThumbnail: {
				title: '이미지 섬네일 설정',
				enabled: '이미지 섬네일 활성화',
				enabledDescription: '카드에 이미지 섬네일을 표시합니다',
				size: '섬네일 크기',
				sizeDescription: '이미지 섬네일의 크기를 선택합니다',
				sizeSmall: '작게 (80px)',
				sizeMedium: '중간 (150px)',
				sizeLarge: '크게 (250px)',
				aspectRatio: '종횡비',
				aspectRatioDescription: '이미지의 가로세로 비율을 선택합니다',
				aspectRatioSquare: '정사각형 (1:1)',
				aspectRatioOriginal: '원본 비율',
				aspectRatio16_9: '와이드 (16:9)',
				aspectRatio4_3: '표준 (4:3)',
				fallback: '폴백 옵션',
				fallbackDescription: '이미지가 없을 때 대신 표시할 내용을 선택합니다',
				fallbackNone: '없음 (섹션 숨김)',
				fallbackIcon: '파일 타입 아이콘',
				fallbackFolderColor: '폴더 기반 색상',
				fallbackTagColor: '태그 기반 색상',
				fallbackEmoji: '첫 번째 이모지',
				allowExternal: '외부 이미지 허용',
				allowExternalDescription: 'http/https URL의 외부 이미지를 표시할지 결정합니다',
				retryCount: '재시도 횟수',
				retryCountDescription: '이미지 로딩 실패 시 재시도 횟수를 설정합니다',
				clickAction: '클릭 동작',
				clickActionDescription: '이미지를 클릭했을 때의 동작을 선택합니다',
				clickOpenFile: '파일 열기',
				clickOpenImage: '이미지 새 탭에서 열기',
				clickNone: '동작 없음',
			},
			...
			inheritFromNormalContentDescription: '활성화하면 일반 상태의 내용을 그대로 사용합니다. 공통 내용을 유지하면서 일부만 변경할 때 유용합니다',
			inheritingFromNormalInfo: '일반 상태의 스타일을 상속받고 있습니다',
			inheritingFromNormalContentInfo: '일반 상태의 내용을 상속받고 있습니다',
			fontSize: '폰트 크기',
		...
		sortSettings: {
			...
			enableMultiSort: '다단계 정렬 활성화',
			enableMultiSortDescription: '여러 기준으로 순차적으로 정렬합니다. 예: 먼저 폴더별로 정렬한 후, 수정일로 정렬, 그 다음 이름순으로 정렬.',
			sortLevels: '정렬 레벨',
			level: '레벨',
			addLevel: '+ 정렬 레벨 추가',
			removeLevel: '제거',
		...
		cacheStatistics: {
			title: '검색 캐시 통계',
			viewNotOpen: 'Card Navigator 뷰를 열어야 캐시 통계를 볼 수 있습니다',
			notAvailable: '캐시 통계를 사용할 수 없습니다',
			...
			l2Size: 'L2 캐시 크기',
			totalSize: '전체 캐시 크기',
			clearCache: '검색 캐시 지우기',
			clearCacheDescription: '모든 캐시된 검색 결과를 삭제합니다. 캐시 통계가 초기화됩니다.',
			clearCacheButton: '캐시 지우기',
			cacheCleared: '검색 캐시가 삭제되었습니다',
		},
	},
	...
	selection: {
		...
		filesPinned: (count: number) => `${count}개 파일이 핀되었습니다`,
		filesUnpinned: (count: number) => `${count}개 파일의 핀이 해제되었습니다`,
		filesStarred: (count: number) => `${count}개 파일이 즐겨찾기에 추가되었습니다`,
		filesUnstarred: (count: number) => `${count}개 파일이 즐겨찾기에서 제거되었습니다`,
		bookmarksNotAvailable: '즐겨찾기 플러그인을 사용할 수 없습니다',
	},
